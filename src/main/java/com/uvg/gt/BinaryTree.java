...
public class BinaryTree<T extends Comparable<T>> implements ITree<T> {
	...
	public void inOrder(BinaryTreeNode<T> nodo_r, int nivel) {
		if (nodo_r == null) {
			...
			// mostrar el nodo "central"
			System.out.println(((Item) nodo_r.getValue()).getLlave());

			// mostar el nodo derecho
			inOrder(nodo_r.getRight(), nivel++);
		}
	}
}